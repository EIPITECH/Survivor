...
function DashboardPcSeeker() {
    ...
    return (
        <div className="flex flex-col gap-6 bg-[#f7f8fb] px-5 py-8 sm:px-10">
            <div>
                <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#1B3A6B]">Espace candidat</p>
                <h1 className="mt-2 text-3xl font-bold text-black">Mon profil</h1>
            </div>

            <section className="flex flex-col gap-5 rounded-2xl bg-white p-6 shadow-[0_0_25px_rgba(0,0,0,0.1)]">
                <h2 className="text-xl font-bold text-black">Candidature</h2>
                <div className="flex flex-col gap-4 rounded-xl border border-gray-100 p-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex gap-3">
                        <div className="border-l-2 border-[#1B3A6B]" />
                        <div>
...
function ProfileField({ label, value }: { label: string; value: string }) {
    return (
        <div className="rounded-lg border border-gray-200 px-4 py-3">
            ...
            <p className="mt-1 font-bold text-[#1B3A6B]">{value}</p>
        </div>
    );
}
